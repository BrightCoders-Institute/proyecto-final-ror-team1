...
class Carriers::TrackingRequesterDhlTest < ActiveSupport::TestCase
  def setup
    ...
    @parser_rule_delivered = ParserRule.create(code: 'delivered', description: 'Delivered', internal_code: 'DELIVERED', carrier_id: @carrier.id)
    @parser_rule_in_transit = ParserRule.create(code: 'transit', description: 'In transit', internal_code: 'IN_TRANSIT', carrier_id: @carrier.id)
    @parser_rule_pre_transit = ParserRule.create(code: 'pre-transit', description: 'Collected', internal_code: 'REGISTERED', carrier_id: @carrier.id)
